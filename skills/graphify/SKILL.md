...
**Before starting:** note whether `--mode deep` was given. Track this from the original invocation — pass it to the Ollama prompt (Path A) or as `DEEP_MODE=true` to every subagent (Path B). Do not lose it.

This step has two parts: **structural extraction** (deterministic, free) and **semantic extraction** (Ollama if configured, Claude subagents as fallback).

**Run Part A (AST) and Part B (semantic) in parallel when using Claude subagents. When using Ollama (Path A), start AST extraction in the same message as the Ollama Python script — both run concurrently. Merge results in Part C as before.**

Note: Parallelizing AST + semantic saves 5-15s on large corpora. AST is deterministic and fast; start it while semantic extraction is running.
...
#### Part B - Semantic extraction (Ollama or Claude subagents)
...
Load files from `graphify-out/.graphify_uncached.txt`. Split into chunks of 20-25 files each for Claude subagents, or 5 files each for Ollama (smaller context window). Each image gets its own chunk (vision needs separate context). When splitting, group files from the same directory together so related artifacts land in the same chunk.

**Step B2 - Choose extraction backend**

Check `PREFLIGHT_EXTRACTION_MODEL` env var to determine the extraction path:

```bash
EXTRACTION_MODEL="${PREFLIGHT_EXTRACTION_MODEL:-}"
OLLAMA_BASE="${OLLAMA_HOST:-http://host.docker.internal:11434}"
USE_OLLAMA=false
OLLAMA_MODEL=""

if [[ "$EXTRACTION_MODEL" == ollama:* ]]; then
    OLLAMA_MODEL="${EXTRACTION_MODEL#ollama:}"
    if curl -sf --max-time 5 "${OLLAMA_BASE}/api/tags" > /dev/null 2>&1; then
        USE_OLLAMA=true
        echo "Extraction backend: Ollama ($OLLAMA_MODEL at $OLLAMA_BASE) — \$0 token cost"
    else
        echo "Ollama configured ($OLLAMA_MODEL) but unreachable at ${OLLAMA_BASE} — falling back to Claude subagents"
    fi
else
    echo "Extraction backend: Claude subagents (set PREFLIGHT_EXTRACTION_MODEL=ollama:<model> to use local Ollama)"
fi
```

**Path A — Ollama extraction (when USE_OLLAMA=true)**

When `PREFLIGHT_EXTRACTION_MODEL=ollama:<model>` and Ollama is reachable, extract via direct Ollama API calls. Zero token cost. Runs sequentially (no subagents needed):

```bash
$(cat graphify-out/.graphify_python) << 'PYEOF'
import json, os, sys, math
from pathlib import Path
import urllib.request

ollama_host = os.environ.get("OLLAMA_HOST", "http://host.docker.internal:11434")
extraction_model_env = os.environ.get("PREFLIGHT_EXTRACTION_MODEL", "")
ollama_model = extraction_model_env.replace("ollama:", "", 1)
deep_mode = os.environ.get("GRAPHIFY_DEEP_MODE", "") == "true"

uncached_text = Path("graphify-out/.graphify_uncached.txt").read_text().strip()
uncached_all = [f for f in uncached_text.split("\n") if f]

# Skip image files — Ollama may not be vision-capable. Write them back for Claude subagent fallback.
IMAGE_EXTS = {".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp", ".tiff"}
image_files = [f for f in uncached_all if Path(f).suffix.lower() in IMAGE_EXTS]
uncached = [f for f in uncached_all if Path(f).suffix.lower() not in IMAGE_EXTS]

if image_files:
    Path("graphify-out/.graphify_image_uncached.txt").write_text("\n".join(image_files))
    print(f"  {len(image_files)} image file(s) deferred to Claude subagents (vision required)")

CHUNK_SIZE = 5  # Smaller chunks for Ollama context window
chunks = [uncached[i:i+CHUNK_SIZE] for i in range(0, len(uncached), CHUNK_SIZE)]
total = len(chunks)
print(f"Ollama extraction: {len(uncached)} files → {total} chunks via {ollama_model}{' [DEEP MODE]' if deep_mode else ''}")

SCHEMA = '{"nodes":[{"id":"filestem_entityname","label":"Human Readable Name","file_type":"code|document|paper|image","source_file":"relative/path","source_location":null,"source_url":null,"captured_at":null,"author":null,"contributor":null}],"edges":[{"source":"node_id","target":"node_id","relation":"calls|implements|references|cites|conceptually_related_to|shares_data_with|semantically_similar_to|rationale_for","confidence":"EXTRACTED|INFERRED|AMBIGUOUS","confidence_score":1.0,"source_file":"relative/path","source_location":null,"weight":1.0}],"hyperedges":[],"input_tokens":0,"output_tokens":0}'

for i, chunk_files in enumerate(chunks):
    chunk_num = i + 1
    out_path = Path(f"graphify-out/.graphify_chunk_{chunk_num:02d}.json")

    file_contents = []
    for fp in chunk_files:
        p = Path(fp)
        if p.exists() and p.stat().st_size < 150_000:
            try:
                file_contents.append(f"=== {fp} ===\n{p.read_text(errors='replace')[:6000]}")
            except Exception as e:
                file_contents.append(f"=== {fp} === [read error: {e}]")

    if not file_contents:
        continue

    deep_note = (
        "\nDEEP MODE: be aggressive with INFERRED edges — indirect deps, shared assumptions, "
        "latent couplings. Mark uncertain ones AMBIGUOUS instead of omitting."
    ) if deep_mode else ""

    prompt = f"""You are a graphify extraction agent. Read the files below and extract a knowledge graph fragment.
...
- Doc/paper files: extract named concepts, entities, rationale (WHY decisions were made)
- Code files: semantic edges AST cannot find (call relationships, shared data, arch patterns){deep_note}

Output exactly this JSON schema (fill with real data):
{SCHEMA}"""
    ...
    try:
        ...
        if start >= 0 and end > start:
            chunk_data = json.loads(response_text[start:end])
            out_path.write_text(json.dumps(chunk_data, indent=2))
            n = len(chunk_data.get("nodes", []))
            e = len(chunk_data.get("edges", []))
            print(f"  chunk {chunk_num}/{total}: {n} nodes, {e} edges")
        else:
            print(f"  chunk {chunk_num}/{total}: WARNING — no valid JSON in Ollama response")
    except Exception as ex:
        print(f"  chunk {chunk_num}/{total}: ERROR — {ex}")

# Merge all chunk files into semantic_new
all_nodes, all_edges, all_hyperedges = [], [], []
for f in sorted(Path("graphify-out").glob(".graphify_chunk_*.json")):
    try:
...
print(f"Ollama extraction complete: {len(all_nodes)} nodes, {len(all_edges)} edges (0 tokens)")
PYEOF
```

After Path A completes, skip to Step B3.

**Path B — Claude subagents (when USE_OLLAMA=false)**

Only used when Ollama is not configured or unreachable. Costs tokens but runs all chunks in parallel.
...
**If Path A (Ollama) was used:** `.graphify_semantic_new.json` is already written. Skip the subagent-wait instructions below and go straight to "Save new results to cache."

**If Path B (Claude subagents) was used:** Wait for all subagents. For each result:
...
**If Path A left image files deferred** (`graphify-out/.graphify_image_uncached.txt` exists): dispatch one Claude subagent per image file now, using the standard extraction prompt with vision. Merge results into the semantic_new before caching.
