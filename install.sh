...
echo "Claude Code Preflight - Install"
echo ""
...
copy_file "$SCRIPT_DIR/scripts/governance-init.sh" "scripts/governance-init.sh"
copy_file "$SCRIPT_DIR/scripts/governance-check.sh" "scripts/governance-check.sh"
copy_file "$SCRIPT_DIR/scripts/memory-promote.sh" "scripts/memory-promote.sh"
copy_file "$SCRIPT_DIR/scripts/agent-gate.sh" "scripts/agent-gate.sh"
copy_file "$SCRIPT_DIR/scripts/adversarial-audit.sh" "scripts/adversarial-audit.sh"
copy_file "$SCRIPT_DIR/scripts/cross-project-ingest.sh" "scripts/cross-project-ingest.sh"
copy_file "$SCRIPT_DIR/scripts/session-synthesize.sh" "scripts/session-synthesize.sh"
copy_file "$SCRIPT_DIR/scripts/govern-onboard.sh" "scripts/govern-onboard.sh"
copy_file "$SCRIPT_DIR/scripts/staleness-scan.sh" "scripts/staleness-scan.sh"
copy_file "$SCRIPT_DIR/scripts/uninstall.sh" "scripts/uninstall.sh"
copy_file "$SCRIPT_DIR/scripts/fresh.sh" "scripts/fresh.sh"
copy_file "$SCRIPT_DIR/scripts/sync-health.sh" "scripts/sync-health.sh"
copy_file "$SCRIPT_DIR/scripts/snapshot.sh" "scripts/snapshot.sh"
copy_file "$SCRIPT_DIR/scripts/context-pack.sh" "scripts/context-pack.sh"
copy_file "$SCRIPT_DIR/scripts/project-ingest.sh" "scripts/project-ingest.sh"
copy_file "$SCRIPT_DIR/scripts/repair-hooks.sh" "scripts/repair-hooks.sh"
copy_file "$SCRIPT_DIR/scripts/idea-log.sh" "scripts/idea-log.sh"
copy_file "$SCRIPT_DIR/scripts/council.sh" "scripts/council.sh"

# Self-heal: drop any orphan hook entries in ~/.claude/settings.json that
# point at hook files that no longer exist (common after lockout recovery
# where the user removed a hook file manually).
if [ -x "$SCRIPT_DIR/scripts/repair-hooks.sh" ]; then
    "$SCRIPT_DIR/scripts/repair-hooks.sh" --apply >/dev/null 2>&1 || true
fi
...
GITIGNORE_ADDS=(
    ...
    ".claude_profile"
    ".agent/.cross-project-ingest-acknowledged"
    ".agent/.last-synthesis"
    ".agent/.gate-approve"
    "audits/gate.log"
    "audits/reports/"
    ".agent/onboarding-answers.yaml"
    "archive/snapshot-*/"
    "archive/fresh-*/"
    "staleness-report.md"
    "governance/councils/"
...
for hook in pre-bash-firewall.sh protect-critical-files.sh post-edit-quality.sh pre-compact-synthesize.sh pre-tool-policy-gate.sh; do
...
desired_pre = {
    ...
    '*':         os.path.join(hooks_dir, 'pre-tool-policy-gate.sh'),
}
desired_post = {
    'Edit|MultiEdit|Write': os.path.join(hooks_dir, 'post-edit-quality.sh'),
}
desired_precompact = {
    'manual': os.path.join(hooks_dir, 'pre-compact-synthesize.sh'),
    'auto':   os.path.join(hooks_dir, 'pre-compact-synthesize.sh'),
}
...
for matcher, cmd in desired_precompact.items():
    upsert_hook(precompact, matcher, cmd)
...
# Install /govern skill
GOVERN_SKILL_DIR="$HOME/.claude/skills/govern"
GOVERN_SKILL_SRC="$SCRIPT_DIR/skills/govern/SKILL.md"
if [ -f "$GOVERN_SKILL_SRC" ]; then
    if [ ! -f "$GOVERN_SKILL_DIR/SKILL.md" ] || [ "$FORCE" = true ]; then
        mkdir -p "$GOVERN_SKILL_DIR"
        if cp "$GOVERN_SKILL_SRC" "$GOVERN_SKILL_DIR/SKILL.md"; then
            ok "Govern skill installed (/govern)"
        else
            warn "Could not install govern skill"
        fi
    else
        ok "Govern skill already installed"
    fi
fi

# Make the governance bundle locatable by scripts/governance-init.sh.
# This sets PREFLIGHT_HOME so the project's scripts know where the
# canonical templates and policies live.
if ! grep -q '^export PREFLIGHT_HOME=' "$HOME/.bashrc" 2>/dev/null; then
    printf '\n# claude_preflight: governance bundle location\nexport PREFLIGHT_HOME="%s"\n' "$SCRIPT_DIR" >> "$HOME/.bashrc"
    ok "PREFLIGHT_HOME exported in .bashrc → $SCRIPT_DIR"
fi
...
# step 5b: install opa so the policy gate can actually evaluate
# Without this, the PreToolUse policy gate runs in advisory mode (warn +
# allow). Installing OPA as part of bootstrap means the gate enforces from
# the first session, no manual step required.
log "Step 5b: Installing OPA (policy engine)..."
if command -v opa >/dev/null 2>&1; then
    ok "OPA already on PATH ($(opa version 2>&1 | head -1))"
else
    OPA_DEST="${HOME}/.local/bin/opa"
    mkdir -p "$(dirname "$OPA_DEST")"
    OPA_URL=""
    case "$(uname -s)" in
        Linux)  OPA_URL="https://openpolicyagent.org/downloads/latest/opa_linux_amd64_static" ;;
        Darwin) OPA_URL="https://openpolicyagent.org/downloads/latest/opa_darwin_amd64" ;;
        *)      warn "Unsupported uname for OPA auto-install: $(uname -s)"; OPA_URL="" ;;
    esac
    if [ -n "$OPA_URL" ]; then
        if curl -sSL -o "$OPA_DEST" "$OPA_URL" && chmod +x "$OPA_DEST"; then
            ok "OPA installed to $OPA_DEST"
            # Also try a system-wide install so cron jobs, sub-shells, and
            # other users see opa on $PATH without needing ~/.local/bin in
            # everyone's environment. Soft-fails if no sudo.
            if [ ! -x /usr/local/bin/opa ]; then
                if sudo -n install -m 0755 "$OPA_DEST" /usr/local/bin/opa 2>/dev/null; then
                    ok "OPA also installed system-wide at /usr/local/bin/opa"
                else
                    warn "Could not install /usr/local/bin/opa (no passwordless sudo)"
                    warn "  If sub-shells / cron / other users need opa, run:"
                    warn "    sudo install -m 0755 $OPA_DEST /usr/local/bin/opa"
                fi
            fi
            ...
            if ! grep -qF '$HOME/.local/bin' "$HOME/.bashrc" 2>/dev/null \
              && ! grep -qF '~/.local/bin' "$HOME/.bashrc" 2>/dev/null; then
                printf '\n# claude_preflight: OPA install location\nexport PATH="$HOME/.local/bin:$PATH"\n' >> "$HOME/.bashrc"
                ok "Added \$HOME/.local/bin to PATH in .bashrc (effective in next shell)"
            fi
            "$OPA_DEST" version 2>&1 | head -1
        else
            warn "Could not download OPA — gate will run in advisory mode until installed manually"
            warn "  Manual: curl -sSL -o $OPA_DEST $OPA_URL && chmod +x $OPA_DEST"
            warn "  System-wide: sudo install -m 0755 $OPA_DEST /usr/local/bin/opa"
        fi
    fi
fi
...
echo ""
PREFLIGHT_RC=0
bash scripts/preflight.sh || PREFLIGHT_RC=$?
if [ "$PREFLIGHT_RC" -ne 0 ]; then
    echo ""
    warn "Preflight reported issues (exit ${PREFLIGHT_RC}). Install completed but the project may need attention."
fi
